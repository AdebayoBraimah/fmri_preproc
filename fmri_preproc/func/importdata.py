...
from fmri_preproc.utils.util import dict2json, json2dict, update_sidecar, timeops

from fmri_preproc.utils.enums import SegType, LogLevel, PhaseEncodeDirection

from fmri_preproc.utils.fileio import File, NiiFile

from fmri_preproc.utils.fslpy import bet, fslmaths, fslreorient2std, fslroi

from fmri_preproc.func.mcdc import motion_outlier, write_slice_order
    ...
    **kwargs,
    ...
    if subid:
        ...
        with NiiFile(src=func, assert_exists=True, validate_nifti=True) as fn:
            ...
            if not subid:
                raise RuntimeError(
                    "No subject ID can be inferred from the data. \
                    The input data should be in BIDS format, and prefixed with \
                    'sub-' in the filename."
                )
    else:
        raise RuntimeError(
            "No Subject (and session) IDs NOR BIDS named functional image file were specified."
        )
    ...
    outputs: Dict[str, Union[int, str]] = {
        "subject_info": os.path.join(info_dir, info_data + "_info.json")
    }

    # Define info dictionary
    info_name: str = outputs.get("subject_info")
    info: Dict[Any, Any] = json2dict(jsonfile=info_name) if os.path.exists(
        info_name
    ) else {}
...
def import_func(
    ...
    with WorkDir(src=outdir) as od:
        ...
        with WorkDir(src=importdir) as impd:
            if log:
                log.log("Creating import data directory")
    ...
    # Create func_mean
    func_mean: str = fslmaths(img=func).Tmean().run(
        out=outputs.get("func_mean"), log=log
    )
    ...
    if func_brainmask:
        with NiiFile(
            ...
            _, func_brainmask, _ = fslreorient2std(
                img=func_brainmask, out=outputs.get("func_brainmask"), log=log
            )
    else:
        ...
        with TmpDir(src=dirname) as tmp:
            ...
            func_brainmask: str = fslmaths(img=brain).bin().run(
                out=outputs.get("func_brainmask"), log=log
            )

    # Create dilated brainmask
    func_dilated_brainmask: str = fslmaths(img=func_brainmask).dilM().dilM().bin().run(
        out=outputs.get("func_dil_brainmask"), log=log
    )
    ...
    _: str = fslmaths(img=func_mean).div(func_std).run(
        out=outputs.get("func_tsnr"), log=log
    )

    # Compute motion metrics and outliers
    _: Tuple[str, int, None] = motion_outlier(
        func=func,
    ...
    if func_slorder:
        ...
        func_slorder: str = write_slice_order(
            slices=slices,
            mb_factor=mb_factor,
            mode="single-shot",
    ...
    if sbref:
        ...
        if sbref_pedir:
            ...
            if log:
                log.error("RuntimeError: sbref' phase-encoding direction required.")
        ...
        if sbref_echospacing:
            ...
            if log:
                log.error("RuntimeError: sbref' echo-spacing required.")
            raise RuntimeError("'sbref' echo-spacing required.")

        with NiiFile(src=sbref, assert_exists=True, validate_nifti=True) as sb:
            _, sbref, _ = fslreorient2std(
                img=sb.abspath(), out=outputs.get("sbref"), log=log
            )
            _: str = update_sidecar(
                file=sbref, echo_spacing=sbref_echospacing, phase_encode_dir=sbref_pedir
            )
        ...
        if sbref_brainmask:
            with NiiFile(
                ...
                _, sbref_brainmask, _ = fslreorient2std(
                    img=sbm.abspath(), out=outputs.get("sbref_brainmask"), log=log
                )
        else:
            with NiiFile(src=func) as fn:
                dirname, _, _ = fn.file_parts()
            ...
            with TmpDir(src=dirname) as tmp:
                ...
                sbref_brainmask: str = fslmaths(img=brain).bin().run(
                    out=outputs.get("sbref_brainmask"), log=log
                )
    return (
        func,
        func_mean,
...
def import_struct(
    ...
    with NiiFile(src=brainmask, assert_exists=True, validate_nifti=True) as bm:
        _, brainmask, _ = fslreorient2std(
            img=bm.abspath(), out=outputs.get("T2w_brainmask"), log=log
        )

    with NiiFile(src=dseg, assert_exists=True, validate_nifti=True) as ds:
        _, dseg, _ = fslreorient2std(
            img=ds.abspath(), out=outputs.get("T2w_dseg"), log=log
        )
        _: str = update_sidecar(file=dseg, dseg_type=dseg_type)

    if probseg:
        with NiiFile(src=probseg, assert_exists=True, validate_nifti=True) as pb:
            _, probseg, _ = fslreorient2std(
                img=pb.abspath(), out=outputs.get("T2w_probseg"), log=log
            )
            _: str = update_sidecar(file=probseg, probseg_type=probseg_type)

    if wmmask:
        with NiiFile(src=wmmask, assert_exists=True, validate_nifti=True) as wm:
            _, wmmask, _ = fslreorient2std(
                img=wm.abspath(), out=outputs.get("T2w_wmmask"), log=log
            )
    else:
        wmmask: str = create_mask(
            dseg=dseg, dseg_type=dseg_type, labels=["wm"], out=outputs.get("T2w_wmmask")
        )

    if T1w:
        with NiiFile(src=T1w, assert_exists=True, validate_nifti=True) as t1:
            _, T1w, _ = fslreorient2std(
                img=t1.abspath(), out=outputs.get("T1w"), log=log
            )

    return (T2w, wmmask, dseg, outputs)
...
def import_spinecho(
    ...
    with TmpDir(src=topup_dir) as tmp:
        ...
        if spinecho:
            if log:
                log.log(f"Input spinecho fieldmap(_epi): {spinecho}.")

            if not spinecho_pedir:
                if log:
                    log.error(
                        f"TypeError: No phase encoding direction list was passed as input."
                    )
            ...
            spinecho_pedir: List[str] = [
                PhaseEncodeDirection(x.upper()).name for x in spinecho_pedir
            ]

            if log:
                log.log(f"Phase encodind directions: {spinecho_pedir}")

            with NiiFile(src=spinecho, assert_exists=True, validate_nifti=True) as sp:
                _, input_spinecho, _ = fslreorient2std(
                    img=sp.abspath(), out=input_spinecho
                )
            ...
            input_spinecho: str = merge_rpe(
                out=input_spinecho, log=log, ap_dir=input_ap, pa_dir=input_pa
            )
            ...
            with NiiFile(src=lr_dir, assert_exists=True, validate_nifti=True) as lrd:
                with NiiFile(
                    src=rl_dir, assert_exists=True, validate_nifti=True
                ) as rld:
            ...
            input_spinecho: str = merge_rpe(
                out=input_spinecho, log=log, lr_dir=input_lr, rl_dir=input_rl
            )
            ...
            with NiiFile(src=is_dir, assert_exists=True, validate_nifti=True) as isd:
                with NiiFile(
                    src=si_dir, assert_exists=True, validate_nifti=True
                ) as sid:
            ...
            input_spinecho: str = merge_rpe(
                out=input_spinecho, log=log, is_dir=input_is, si_dir=input_si
            )

        else:
            if log:
                log.error(
                    f"AttributeError: Input images are not reversed phase encoded fieldmap EPIs: Spinecho: {spinecho} \nAP: {ap_dir} \nPA: {pa_dir} \nLR: {lr_dir} \nRL: {rl_dir} \nIS: {is_dir} \nSI: {si_dir}"
                )
            raise AttributeError(
                f"Input images are not reversed phase encoded fieldmap EPIs: Spinecho: {spinecho} \nAP: {ap_dir} \nPA: {pa_dir} \nLR: {lr_dir} \nRL: {rl_dir} \nIS: {is_dir} \nSI: {si_dir}"
            )
