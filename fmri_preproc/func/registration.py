...
from fmri_preproc.utils.enums import BBRType, PhaseEncodeDirection

from fmri_preproc.utils.acqparam import _get_axis as get_axis, phase_encode_dict
...
def fmap_to_struct(
    ...
    with NiiFile(src=fmap, assert_exists=True, validate_nifti=True) as fmp:
        with NiiFile(
            src=fmap_magnitude, assert_exists=True, validate_nifti=True
        ) as fmm:
            with NiiFile(
                src=fmap_brainmask, assert_exists=True, validate_nifti=True
            ) as fmb:
    ...
    if struct_boundarymask:
        with NiiFile(
            src=struct_boundarymask, assert_exists=True, validate_nifti=True
        ) as sbm:
    ...
    if bbr:
        ...
        if isinstance(bbr_slope, float) or isinstance(bbr_slope, int):
            ...
            if log:
                log.error(
                    f"TypeError: Input 'bbr_slope': {bbr_slope} is not an 'int' nor 'float'."
                )
            raise TypeError(
                f"Input 'bbr_slope': {bbr_slope} is not an 'int' nor 'float'."
            )
    ...
    with TmpDir(src=regdir) as tmp:
        ...
        struct_brain: str = fslmaths(img=struct).mas(img=struct_brainmask).run(
            out=struct_brain, log=log
        )

        fmap_brain: str = os.path.join(tmp.src, "fmap_brain.nii.gz")
        fmap_brain: str = fslmaths(img=fmap_magnitude).mas(img=fmap_brainmask).run(
            out=fmap_brain, log=log
        )
        ...
        (affine, inv_affine, src2ref, _, _) = epireg(**kwargs)

    return (affine, inv_affine, src2ref, init_affine)
...
def func_to_sbref(
    ...
    with NiiFile(src=sbref, assert_exists=True, validate_nifti=True) as sb:
        with NiiFile(
            src=sbref_brainmask, assert_exists=True, validate_nifti=True
        ) as sm:
    ...
    with TmpDir(src=regdir) as tmp:
        ...
        func_brain: str = fslmaths(img=func).mas(func_brainmask).run(
            out=func_brain, log=log
        )

        sbref_brain: str = os.path.join(tmp.src, "sbref_brain.nii.gz")
        sbref_brain: str = fslmaths(img=sbref).mas(sbref_brainmask).run(
            out=sbref_brain, log=log
        )
        ...
        (affine, inv_affine, src2ref, _, _) = epireg(
            outdir=tmp.src,
            src=func,
            ref=sbref,
            src_space=src_space,
...
def sbref_to_struct(
    ...
    if dc:
        if not _has_fmap:
            raise RuntimeError(
                "'fmap' and 'fmap_brainmask' are required for distortion correction."
            )

        if not _has_sbref_props:
            raise RuntimeError(
                "'sbref_pedir' and 'sbref_echospacing' are required for distortion correction."
            )
    ...
    with TmpDir(src=regdir) as tmp:
        ...
        struct_brain: str = fslmaths(img=struct).mas(struct_brainmask).run(
            out=struct_brain, log=log
        )
        ...
        (affine, inv_affine, src2ref, warp, _) = epireg(**epireg_kwargs)

        # Create warp to distortion correct func and/or sbref/sbref_brainmask
        if dc:
            dc_warp: str = convertwarp(
                warp1=warp, postmat=inv_affine, ref=sbref, out=outputs.get("dc_warp")
            )

            dc_img: str = applywarp(
                src=sbref,
                ref=sbref,
                warp=dc_warp,
            ...
            dc_brainmask: str = fslmaths(img=dc_brainmask).thr(0.5).bin().run(
                out=dc_brainmask, log=log
            )
        else:
            dc_warp: str = None
            dc_img: str = None
    ...
    return (affine, inv_affine, src2ref, warp, dc_warp, dc_img, dc_brainmask)
...
def func_to_struct_composite(
    ...
    inv_affine: str = invxfm(inmat=affine, outmat=outputs.get("inv_affine"), log=log)

    if _has_warp:
        with NiiFile(
            src=sbref2struct_warp, assert_exists=True, validate_nifti=True
        ) as sbs:
            sbref2struct_warp: str = sbs.abspath()

        # Func (distorted) -> struct :: warp
        warp: str = convertwarp(
        ...
        inv_warp: str = invwarp(
            inwarp=warp, ref=func, outwarp=outputs.get("inv_warp"), log=log
        )
        ...
        dc_img: str = applywarp(
            src=func, ref=func, warp=dc_warp, out=outputs.get("dc_image"), log=log
        )

        # Resample func -> structural space
        resamp_img: str = applywarp(
            src=func,
            ref=struct,
            warp=warp,
    ...
    return (affine, inv_affine, warp, inv_warp, dc_warp, dc_img, resamp_img)
...
def fmap_to_func_composite(
    ...
    with NiiFile(src=fmap, assert_exists=True, validate_nifti=True) as fm:
        with NiiFile(src=func, assert_exists=True, validate_nifti=True) as fn:
            ...
            if not src_space:
                _, src_space, _ = fm.file_parts()
            if not ref_space:
                _, ref_space, _ = fn.file_parts()
...
def _select_atlas(
    ...
    try:
        ...
        if (age.lower() == "neo") or (age.lower() == "neonate"):
            ...
            raise RuntimeError(
                f"The input for age is insufficient: {age}. Use 'neo' or 'neonate' or the PMA in weeks."
            )
    ...
    if atlasdir:
        ...
        atlasdir: str = " ".join(
            map(str, glob.glob(os.path.join(ATLASDIR, "UNC*2020*", "atlas")))
        )
    elif (28 <= age) and (age <= 44):
        atlasdir: str = " ".join(
            map(str, glob.glob(os.path.join(ATLASDIR, "*dhcp*atlas*", "atlas")))
        )
    else:
    ...
    with WorkDir(src=atlasdir) as ad:
        ...
        warpsdir: str = ad.join("warps")

        with WorkDir(src=templatedir) as td:
            with WorkDir(src=warpsdir) as wd:
                templatedir: str = td.abspath()
                warpsdir: str = wd.abspath()
                ...
                xfm: str = " ".join(
                    map(str, glob.glob(f"*{age}*to*{standard_age}_warp*.nii*"))
                )
                invxfm: str = " ".join(
                    map(str, glob.glob(f"*{age}*to*{standard_age}_invwarp*.nii*"))
                )
                ...
                if t2 == "":
                    raise FileNotFoundError(
                        f"Unable to find T2w image template for the specified age: {age} and atlas directory: {td.abspath()}"
                    )
                else:
                    t2: str = td.join(t2)
...
def template_to_struct(
    ...
    if struct_T2w and atlas.get("template_T2"):
        with NiiFile(src=struct_T2w, assert_exists=True, validate_nifti=True) as st2:
            with NiiFile(
                src=atlas.get("template_T2"), assert_exists=True, validate_nifti=True
            ) as tt2:
    ...
    if struct_T1w and atlas.get("template_T1"):
        with NiiFile(src=struct_T1w, assert_exists=True, validate_nifti=True) as st1:
            with NiiFile(
                src=atlas.get("template_T1"), assert_exists=True, validate_nifti=True
            ) as tt1:
    ...
    if struct_gmprob:
        ...
        if "dchp" in atlas.get("prob").lower():
            with NiiFile(
                src=atlas.get("prob"), assert_exists=True, validate_nifti=True
            ) as tgmpb:
                tprob: str = tgmpb.abspath()
                gmpb: str = os.path.join(regdir, "template_gmprob.nii.gz")
                gmpb: str = fslroi(img=tprob, out=gmpb, tmin=1, tsize=1, log=log)
                gmpb: str = fslmaths(img=gmpb).div(100).run(out=gmpb, log=log)
        else:
            with NiiFile(
                src=atlas.get("gm_prob"), assert_exists=True, validate_nifti=True
            ) as tgmpb:
    ...
    if struct_wmprob:
        ...
        if "dhcp" in atlas.get("prob").lower():
            with NiiFile(
                ...
                wmpb: str = fslroi(img=tprob, out=wmpb, tmin=1, tsize=1, log=log)
                wmpb: str = fslmaths(img=wmpb).div(100).run(out=wmpb, log=log)
        else:
            with NiiFile(
                src=atlas.get("wm_prob"), assert_exists=True, validate_nifti=True
            ) as twmpb:
    ...
    (warp, inv_warp, affine, src2ref) = nonlinear_reg(
        src=template,
        ref=struct,
        quick=quick,
        outdir=outdir,
    ...
    inv_warp: str = invwarp(inwarp=warp, ref=template[0], outwarp=inv_warp, log=log)

    return (warp, inv_warp, affine, src2ref)
...
def struct_to_template_composite(
    ...
    standard_atlas: Dict[str, str] = _select_atlas(
        age=standard_age, atlasdir=standarddir, standard_age=standard_age
    )
    ...
    with NiiFile(
        src=standard_atlas.get("template_T2"), assert_exists=True, validate_nifti=True
    ) as std:
        with NiiFile(
            src=atlas.get("template_T2"), assert_exists=True, validate_nifti=True
        ) as ats:
            ...
            if atlas.get("atlas_name") == standard_atlas.get("atlas_name"):
                if not ref_space:
                    _, ref_space, _ = std.file_parts()
            else:
                if not ref_space:
                    _, ref_space, _ = ats.file_parts()

    with NiiFile(src=struct, assert_exists=True, validate_nifti=True) as sct:
        with NiiFile(
            src=struct2template_warp, assert_exists=True, validate_nifti=True
        ) as stw:
            struct: str = sct.abspath()
            struct2template_warp: str = stw.abspath()

            if not src_space:
                _, src_space, _ = sct.file_parts()
    ...
    if log:
        log.log("Create struct2standard warp")
    ...
    if (age != standard_age) and (
        ...
        with NiiFile(src=struct2template_warp) as stw:
            warp: str = stw.sym_link(dst=outputs.get("warp"), relative=True)

    # Invert warp
    inv_warp: str = invwarp(
        inwarp=warp, ref=struct, outwarp=outputs.get("inv_warp"), log=log
    )

    # Resample struct -> template space
    resamp_img: str = applywarp(
        src=struct,
        ref=standard,
        warp=warp,
...
def func_to_template_composite(
    ...
    if log:
        log.log("Create func2standard warp")
    ...
    **kwargs,
    ...
    with NiiFile(src=src, assert_exists=True, validate_nifti=True) as sr:
        with NiiFile(src=ref, assert_exists=True, validate_nifti=True) as rf:
            ...
            ref: str = rf.abspath()
            if not src_space:
                _, src_space, _ = sr.file_parts()
            if not ref_space:
                _, ref_space, _ = rf.file_parts()

    if log:
        log.log(f"Performing epireg: Register {src} to {ref} ")
    ...
    # Logic tests
    _has_fmap: bool = (fmap is not None) and (fmap_brainmask is not None) and (
        fmap2ref_xfm is not None
    )
    _do_bbr: bool = (ref_boundarymask is not None)
    _do_dc: bool = _has_fmap and (src_pedir is not None) and (
        src_echospacing is not None
    )

    if log:
        log.debug(f"Has fieldmap: {_has_fmap}")
    if log:
        log.debug(f"Perform BBR: {_do_bbr}")
    if log:
        log.debug(f"Perform distortion correction: {_do_dc}")
    ...
    with TmpDir(src=regdir) as tmp:
        ...
        if src_brainmask:
            ...
            src_brain: str = fslmaths(img=src).mas(src_brainmask).run(
                out=src_brain, log=log
            )

        if ref_brainmask:
            ref_brain: str = os.path.join(tmp.src, "ref_brain.nii.gz")
            ref_brain: str = fslmaths(img=ref).mas(ref_brain).run(
                out=ref_brain, log=log
            )
        ...
        if _do_dc:
            ...
            fmap_brainmask_to_ref: str = os.path.join(
                tmp.src, "brainmask_to_ref.nii.gz"
            )

            # fmap_to_ref
            fmap_to_ref: str = applyxfm(
                src=fmap,
                ref=ref,
                mat=fmap2ref_xfm,
                out=fmap_to_ref,
            ...
            fmap_brainmask_to_ref: str = applyxfm(
                src=fmap_brainmask,
                ref=ref,
                mat=fmap2ref_xfm,
        ...
        if not _do_dc:
            ...
            src2ref: str = applyxfm(
                src=src, ref=ref, mat=affine, out=src2ref, interp="spline", log=log
            )
        ...
        fmap2src: str = fslmaths(img=fmap2src).mul(fmap2src_mask).run(
            out=fmap2src, log=log
        )  # Re-mask
        ...
        inv_warp: str = invwarp(inwarp=warp, ref=src, outwarp=inv_warp, log=log)

        src2ref: str = outputs.get("src2ref")
        src2ref: str = applywarp(
            src=src, ref=ref, warp=warp, out=src2ref, interp="spline", log=log
        )
    return affine, inv_affine, src2ref, warp, inv_warp
...
def nonlinear_reg(
    ...
    **kwargs: Dict[str, str],
    ...
    if log:
        log.log(f"Performing nonlinear registration: Register {src} to {ref} ")
        ...
        with NiiFile(src=src[0]) as sr:
            with NiiFile(src=ref[0]) as rf:
                with WorkDir(src=outdir) as od:
                    ...
                    if not ref_space:
                        _, ref_space, _ = rf.file_parts()
                    regdir: str = os.path.join(
                        od.src, "reg", f"{src_space}_to_{ref_space}"
                    )

                    with WorkDir(src=regdir) as rd:
                        outdir: str = od.abspath()
                        regdir: str = rd.abspath()
                        basename: str = os.path.join(
                            regdir, f"{src_space}_to_{ref_space}"
                        )
    ...
    cmd.opt(f"{src[0]}")
    cmd.opt("-itk")
    cmd.opt(f"{outputs.get('antsout')}0GenericAffine.mat")
    cmd.opt("-ras2fsl")
    cmd.opt("-o")
    cmd.opt(f"{outputs.get('affine')}")
    cmd.run(log=log)
    ...
    with TmpDir(src=regdir) as tmp:
        ...
        cmd.opt("-mcs")
        cmd.opt(f"{outputs.get('antsout')}1Warp.nii.gz")
        cmd.opt("-oo")
        cmd.opt(f"{tmp.src}/wx.nii.gz")
        cmd.opt(f"{tmp.src}/wy.nii.gz")
        cmd.opt(f"{tmp.src}/wz.nii.gz")
        cmd.run(log=log)

        tmp_invwarp: str = fslmaths(img=f"{tmp.src}/wy.nii.gz").mul(input=int(-1)).run(
            out=f"{tmp.src}/i_wy.nii.gz", log=log
        )
    ...
    warp: str = convertwarp(ref=ref[0], premat=affine, warp1=warp, out=warp, log=log)

    inv_warp: str = invwarp(
        inwarp=warp, outwarp=outputs.get("inv_warp"), ref=src[0], log=log
    )
...
@timeops(log)
def antsRegistrationSyN(
    fixed: Union[List[str], str],
    ...
    if log:
        log.log(
            f"Performing nonlinear registration with antsRegistrationSyN(Quick).sh: Register {fixed} to {moving} "
        )

    assert len(fixed) == len(
        moving
    ), "The same number of fixed and moving images must be provided"
