...
from fmri_preproc.utils.enums import MotionMetric, PhaseEncodeDirection, SliceAcqOrder
...
def mcdc(
    ...
    if (dc & use_mcflirt) | (s2v & use_mcflirt) | (mbs & use_mcflirt):
        if log:
            log.warning(
                "WARNING: MCFLIRT enabled. This has disabled dc, s2v and/or mbs."
            )
    ...
    if dc and not (_has_fmap & _has_acqp):
        if log:
            log.error(
                "RuntimeError: fmap, fmap2func_affine, func_pedir, and func_echospacing are required for DC."
            )
        raise RuntimeError(
            "fmap, fmap2func_affine, func_pedir, and func_echospacing are required for DC."
        )

    if mbs and not (_has_fmap & _has_acqp):
        if log:
            log.error(
                "RuntimeError: fmap, fmap2func_affine, func_pedir, and func_echospacing are required for mbs."
            )
        raise RuntimeError(
            "fmap, fmap2func_affine, func_pedir, and func_echospacing are required for mbs."
        )

    if mbs and not dc:
        if log:
            log.error(
                "RuntimeError: Cannot mbs without dc as they are both distortion corrections."
            )
        raise RuntimeError(
            "Cannot mbs without dc as they are both distortion corrections."
        )
    ...
    # Perform MCDC
    if use_mcflirt:
        (func_mcdc, motfile, _) = mcflirt_mc(
            func=func, func_mc=outputs.get("func_mcdc"), ref=ref, log=log
        )
        eddy_output_mask: str = ""
    else:
        (func_mcdc, motfile, eddy_output_mask) = eddy_mcdc(
            func=func,
    ...
    with TmpDir(src=mcdir) as tmp:
        ...
        brain, _ = bet(
            img=mcdc_mean, out=brain, mask=False, frac_int=0.4, robust=True, log=log
        )
    ...
    if os.path.exists(eddy_output_mask):
        ...
        fov_mask: str = fslmaths(img=mcdc_brainmask).sub(eddy_output_mask).bin().run(
            out=fov_mask, log=log
        )
        fov_percent: str = fslmaths(img=fov_mask).Tmean().mul(100).run(
            out=fov_percent, log=log
        )
    else:
        fov_mask: str = None
...
def mcflirt_mc(
    ...
    (func_mc, parfile, matsdir) = mcflirt(
        infile=func.abspath(),
        outfile=func_mc,
        reffile=ref_file,
        refvol=ref_vol,
    ...
    if dc_warp:
        with TmpDir(src=outdir) as tmp:
            ...
            func_mc: str = applywarp(
                src=func_mc, ref=func_ref, out=func_mc, warp=dc_warp, interp="spline"
            )
    return (func_mc, parfile, matsdir)
...
def eddy_mcdc(
    ...
    NOTE: 
        Input ``fmri`` acquisition is **ASSUMED** to be acquired in the same phase-encoding direction
    ...
    if s2v_corr and (mb_factor or func_sliceorder):
        if func_sliceorder:
            ...
            func_sliceorder: str = write_slice_order(
                slices=slices,
                mb_factor=mb_factor,
                mode="single-shot",
    ...
    (eddy_corr, eddy_motion_par, eddy_mask) = eddy(
        img=func,
        out=eddy_basename,
        mask=func_brainmask,
        acqp=acqp,
    ...
    nib.Nifti1Image(
        nib.load(func_mcdc).get_fdata(), header=func.header, affine=func.affine
    ).to_filename(func_mcdc)

    return (func_mcdc, mot_params, eddy_mask)


def write_bvals(num_frames: int, out_file: str = "file.bval") -> str:
    ...
    np.savetxt(
        out_file,
        np.zeros((1, num_frames), dtype=np.int8, order="C"),
        fmt="%i",
        delimiter=" ",
        encoding="utf-8",
    )
...
def write_bvecs(num_frames: int, out_file: str = "file.bvec") -> str:
...
def write_index(num_frames: int, out_file: str = "file.idx") -> str:
...
def write_slice_order(
    ...
    NOTE:
        - This slice-ordering scheme has only been validated on Philips MR scanners. Use with caution if MR image data is from other vendors.
        - Links for details:
            - `Documentation <https://fsl.fmrib.ox.ac.uk/fsl/fslwiki/eddy/UsersGuide#A--slspec>`_
            - `Implementation (neonatal rs-fMRI) <https://git.fmrib.ox.ac.uk/seanf/dhcp-neonatal-fmri-pipeline/-/blob/master/dhcp/resources/default_func.slorder>`_
            - `Implementation (neonatal dMRI) <https://git.fmrib.ox.ac.uk/matteob/dHCP_neo_dMRI_pipeline_release/-/blob/master/slorder.txt>`_
        - This wrapper function was written with help from `Gregory Lee, PhD <https://github.com/grlee77>`_.
    ...
    if not isinstance(slices, int) and not isinstance(mb_factor, int):
        raise TypeError(
            f"Input option slices: {slices} or mb_factor: {mb_factor} is not an integer."
        )
...
def _refrms(img: nib.Nifti1Image, ref: Optional[nib.Nifti1Image] = None) -> np.array:
    """Calculates REFRMS.
    """
    if ref:
        pass
