...
def postprocess(
    ...
    if isinstance(spatial_fwhm, int) or isinstance(spatial_fwhm, float):
        ...
        if log:
            log.error(
                f"RuntimeError: Input spatial smoothing kernel is neither an integer nor a float: {spatial_fwhm}"
            )
        raise RuntimeError(
            f"Input spatial smoothing kernel is neither an integer nor a float: {spatial_fwhm}"
        )
    ...
    func_smooth: str = fslmaths(img=func).mas(func_brainmask).run(
        out=func_smooth, log=log
    )
    ...
    if spatial_fwhm > 0.01:
        ...
        if log:
            log.log(
                f"Performing spatial smoothing with spatial smoothing kernel (mm): {spatial_fwhm}"
            )
        ...
        func_smooth: str = fslmaths(img=func_smooth).mas(func_brainmask).run(
            out=func_smooth
        )
    ...
    if intnorm:
        ...
        if log:
            log.log(f"Performing grand-mean intensity normalization")
        scaling: float = normmean / median_intensity
        func_intnorm: str = fslmaths(img=func).mul(scaling).run(
            out=func_intnorm, log=log
        )
