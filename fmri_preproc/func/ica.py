...
from fmri_preproc.utils.fslpy import fslmaths, melodic
...
def ica(
    ...
    if temporal_fwhm:
        ...
        with TmpDir(src=icadir) as tmp:
            ...
            func_filt: str = fslmaths(img=func).bptf(
                high_pass=temporal_fwhm, low_pass=-1, tr=func_tr, input_is_sec=True
            ).add(input=func_mean).run(out=func_filt, log=log)
    else:
        with NiiFile(src=func, assert_exists=True, validate_nifti=True) as fn:
